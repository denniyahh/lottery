...
async function initContracts() {
  ...
  const ownerLottery = await contract.read.owner();
  console.log(`Owner: ${ownerLottery}\n`);
  ...
  const ownerHasRole = await tokenContract.read.hasRole([MINTER_ROLE, ownerLottery]);
  console.log(`Owner has minter role: ${ownerHasRole ? 'yes' : 'no'}\n`);
  const lotteryHasRole = await tokenContract.read.hasRole([MINTER_ROLE, contractAddress]);
  console.log(`Lottery contract has minter role: ${lotteryHasRole ? 'yes' : 'no'}\n`);
}
...
async function checkState() {
  ...
  console.log(`Last block was mined at ${currentBlockDate.toLocaleDateString()} : ${currentBlockDate.toLocaleTimeString()}\n`);
...
async function burnTokens(index: string, amount: string) {
  ...
  const receipt = await publicClient.getTransactionReceipt({ hash: allowTx });
