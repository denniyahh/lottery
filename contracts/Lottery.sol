...
contract Lottery is Ownable {
    ...
    bool public betsOpen;
        /// @notice Timestamp when the bets will be closed
    uint256 public betsClosingTime;

        /// @dev List of bet slots
    address[] public _slots;
        /// @dev Mapping of addresses to their prize pool
    ...
    /// @notice Passes when the lottery is at a closed state
    modifier whenBetsClosed() {
        require(!betsOpen, "Lottery is open");
        _;
    }

    /// @notice Passes when the lottery is at an open state and the current block timestamp is lower than the lottery closing time
    ...
    function purchaseTokens() external payable {
        ...
        // uint256 volumeToGive = msg.value / purchasePrice;
        // uint256 change = msg.value - volumeToGive * purchasePrice;
        // paymentToken.mint(msg.sender, volumeToGive);
